...
// burnDispatcher implements dispatcher of new native TPC burns on the blockchain.
...
func (bud *burnDispatcher) process(tx *eventTrx, burn *types.TpcBurn) *types.TpcBurn {
	// no previous burn to check against? make a new record
	if burn == nil {
		return &types.TpcBurn{
	...
	if tx.blk.Number != burn.BlockNumber {
		...
		log.Debugf("collected block burn of %.4f TPC at #%d", val, burn.BlockNumber)

		if err := repo.StoreTpcBurn(burn); err != nil {
...
// burnedFee calculates the amount of burned TPCs from the transaction fee.
