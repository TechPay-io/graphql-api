...
type DefiToken {
    ...
    # canWrapTPC signals if the token can be used
    # to wrap native TPC tokens for DeFi trading.
    canWrapTPC: Boolean!
...
# Transaction is an Photon block chain transaction.
type Transaction {
...
# Block is an Photon block chain block.
type Block {
...
# Delegation represents a delegation on Photon block chain.
type Delegation {
    ...
    # outstandingSTPC represents the amount of sTPC tokens representing
    # the tokenized stake minted and un-repaid on this delegation.
    outstandingSTPC: BigInt!

    # tokenizerAllowedToWithdraw indicates if the stake tokenizer allows the stake
    # to be withdrawn. That means all the sTPC tokens have been repaid and the sTPC
...
# Address is a 20 byte Photon address, represented as 0x prefixed hexadecimal number.
scalar Address
...
# TpcBlockBurn represents a native TPC tokens burn record per created block.
type TpcBlockBurn {
    ...
    # amount represents the amount of TPC tokens burned in WEI units (18 digits fixed INT encoded as HEX number).
    amount: BigInt!

    # tpcValue represents TPC value of the burned TPC tokens.
    tpcValue: Float!
}
# EstimatedRewards represents a calculated rewards estimation for an account or amount staked
type EstimatedRewards {
    # Amount of TPC tokens expected to be staked for the calculation.
    staked: Long!

    # dailyReward represents amount of TPC tokens estimated
    # to be rewarded for staked amount in average per day.
    dailyReward: BigInt!

    # weeklyReward represents amount of TPC tokens estimated
    # to be rewarded for staked amount in average per week.
    weeklyReward: BigInt!

    # monthlyReward represents amount of TPC tokens estimated
    # to be rewarded for staked amount in average per month.
    monthlyReward: BigInt!

    # yearlyReward represents amount of TPC tokens estimated
    ...
    # Total amount of staked TPC tokens used for the calculation in WEI units.
    ...
    # Information about the last sealed epoch of the Photon blockchain.
    # The epoch provides useful information about total TPC supply,
...
# Price represents price information of core Photon token
type Price {
    "Source unit symbol."
    fromSymbol: String!
...
type Query {
    ...
    # Total number of accounts active on the Photon blockchain.
    accountsActive:Long!
    ...
    # Get the id of the current epoch of the Photon blockchain.
    currentEpoch:Long!
    ...
    # The last staker id in Photon blockchain.
    lastStakerId: Long!

    # The number of stakers in Photon blockchain.
    stakersNum: Long!
    ...
    # Get price details of the Photon blockchain token for the given target symbols.
    price(to:String!):Price!

    # Get calculated staking rewards for an account or given
    # staking amount in TPC tokens.
    ...
    # by the Uniswap Core contract on Photon blockchain.
    ...
    # tpcBurnedTotal provides the total amount of native TPC tokens burned
    # by the chain from paid transaction fees in WEI units.
    tpcBurnedTotal: BigInt!

    # tpcBurnedTotalAmount provides the total amount of native TPC tokens burned
    # by the chain from paid transaction fees in TPC units.
    tpcBurnedTotalAmount: Float!

    # tpcLatestBlockBurnList provides a list of latest burned native TPC tokens per-block.
    tpcLatestBlockBurnList(count: Int = 25): [TpcBlockBurn!]!
}
