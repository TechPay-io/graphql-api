...
// OutstandingSTPC resolves the amount of outstanding sTPC tokens
// minted for this account.
func (del Delegation) OutstandingSTPC() (hexutil.Big, error) {
	val, err := repository.R().DelegationOutstandingSTPC(&del.Address, del.Delegation.ToStakerId)
	if err != nil {
		return hexutil.Big{}, err
	}
	return *val, nil
