...
func (tpc *TpcBridge) Transaction(hash *common.Hash) (*types.Transaction, error) {
	// keep track of the operation
	tpc.log.Debugf("loading transaction %s", hash.String())

	// call for data
	var trx types.Transaction
	err := tpc.rpc.Call(&trx, "tpc_getTransactionByHash", hash)
	if err != nil {
		tpc.log.Error("transaction could not be extracted")
		return nil, err
	}
	...
	if trx.BlockNumber != nil {
		...
		err := tpc.rpc.Call(&rec, "tpc_getTransactionReceipt", hash)
		if err != nil {
			tpc.log.Errorf("can not get receipt for transaction %s", hash)
			return nil, err
		}
	...
	// keep track of the operation
	tpc.log.Debugf("transaction %s loaded", hash.String())
	return &trx, nil
}

// SendTransaction sends raw signed and RLP encoded transaction to the block chain.
func (tpc *TpcBridge) SendTransaction(tx hexutil.Bytes) (*common.Hash, error) {
	// keep track of the operation
	tpc.log.Debug("sending new transaction to block chain")

	var hash common.Hash
	err := tpc.rpc.Call(&hash, "eth_sendRawTransaction", tx)
	if err != nil {
		tpc.log.Error("transaction could not be sent")
		return nil, err
	}

	// keep track of the operation
	tpc.log.Debugf("transaction has been accepted with hash %s", hash.String())
	return &hash, nil
}
