...
func (tpc *TpcBridge) DefiTokens() ([]types.DefiToken, error) {
	// connect the contract
	contract, err := tpc.fMintCfg.tokenRegistryContract()
	if err != nil {
		return nil, err
	}

	return tpc.defiTokensList(contract)
}

// DefiTokenList creates a list of addresses / identifiers of all the ERC20 tokens
// involved with the fMint protocol.
func (tpc *TpcBridge) DefiTokenList() ([]common.Address, error) {
	// connect the contract
	contract, err := tpc.fMintCfg.tokenRegistryContract()
	if err != nil {
		return nil, err
	...
	return tpc.defiTokenAddressList(contract.TokensCount, contract.TokensList)
}

// DefiToken loads details of a single DeFi token by it's address.
func (tpc *TpcBridge) DefiToken(token *common.Address) (*types.DefiToken, error) {
	// connect the contract
	contract, err := tpc.fMintCfg.tokenRegistryContract()
	if err != nil {
		return nil, err
	...
	return tpc.defiTokenDetail(contract, token)
}
...
func (tpc *TpcBridge) defiTokenAddressList(
	...
	if err != nil {
		tpc.log.Errorf("can not get tokens range; %s", err.Error())
		return nil, err
	}
	...
	for i := uint64(0); i < count.Uint64(); i++ {
		...
		if err != nil {
			tpc.log.Errorf("token %d address not found; %s", i, err.Error())
			return nil, err
		}
	}
...
func (tpc *TpcBridge) defiTokenDetail(contract *contracts.DefiFMintTokenRegistry, token *common.Address) (*types.DefiToken, error) {
	// get the token details
	tk, err := contract.Tokens(nil, *token)
	if err != nil {
		tpc.log.Errorf("token %s not found; %s", token.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		tpc.log.Errorf("can not decode token %s; %s", token.String(), err.Error())
		return nil, err
	}
...
func (tpc *TpcBridge) defiTokensList(contract *contracts.DefiFMintTokenRegistry) ([]types.DefiToken, error) {
	// get tge list of addresses
	al, err := tpc.defiTokenAddressList(contract.TokensCount, contract.TokensList)
	if err != nil {
		tpc.log.Errorf("tokens list not available; %s", err.Error())
		return nil, err
	}
	...
	for i, addr := range al {
		// decode the token
		tk, err := tpc.defiTokenDetail(contract, &addr)
		if err != nil {
			tpc.log.Errorf("invalid token #%d; %s", i, err.Error())
			return nil, err
		}
