...
// colBurns represents the name of the native TPC burns collection in database.
...
// StoreBurn stores the given native TPC burn record.
func (db *MongoDbBridge) StoreBurn(burn *types.TpcBurn) error {
	if burn == nil {
		return nil
	}
	...
	if sr.Err() != nil {
		...
		db.log.Errorf("could not load TPC burn at #%d; %s", burn.BlockNumber, sr.Err())
		return sr.Err()
	}

	// decode existing burn and update
	var ex types.TpcBurn
	if err := sr.Decode(&sr); err != nil {
		db.log.Errorf("could not decode TPC burn at #%d; %s", burn.BlockNumber, sr.Err())
		return sr.Err()
	}
...
// BurnList provides list of native TPC burns per blocks stored in the persistent database.
func (db *MongoDbBridge) BurnList(count int64) ([]types.TpcBurn, error) {
	col := db.client.Database(db.dbName).Collection(colBurns)
	...
	list := make([]types.TpcBurn, 0, count)

	for cr.Next(ctx) {
		var row types.TpcBurn
		if err := cr.Decode(&row); err != nil {
