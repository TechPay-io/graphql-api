...
import (
	...
	"techpay-api-graphql/internal/repository/db"
...
func connect(cfg *config.Config, log logger.Logger) (*cache.MemBridge, *db.MongoDbBridge, *rpc.TpcBridge, error) {
	...
	// create new Sirius RPC bridge
	rpcBridge, err := rpc.New(cfg, log)
	if err != nil {
		log.Criticalf("can not connect Sirius RPC interface, %s", err.Error())
		return nil, nil, nil, err
	}
